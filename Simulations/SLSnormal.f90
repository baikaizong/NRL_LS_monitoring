Module constants  
  implicit none 
  integer, parameter :: sample_size=100,simu=10000,ewma=50,level=4,miu=0.0 
  real, parameter :: threshhold_a1=-0.7+0.0,threshhold_a2=1.0+0.0,threshhold_a3=1.8+0.0
...
program main
	...
	real shiftm,shifts
     ...
     open(10,file="D:/Location-Scale-Ordinal/Fortran_locationscale/miutxt/SLSnormal_1027-L1N100.txt")
...
subroutine OCARL(Limit,shiftm,shifts,arl,std)
	...
	do i=1,simu
      ...
      call RNGET (iseed)  
      call RNNOR (sample_size, sample)
	  sample=(sample+miu+shiftm)*(1+shifts)
      call ordinal_sample(sample,sample_ordinal)
      sample_ewma=lamda*sample_ordinal+co_lamda*sample_ewma
      call SLSTest(Prob,sample_ewma,Sigmascore,Test)
	  !print*,Test
...
subroutine nomICProb(Prob)
	...
	interval(1)=ANORDF(threshhold_a1-miu)
	interval(2)=ANORDF(threshhold_a2-miu)
	interval(3)=ANORDF(threshhold_a3-miu)
