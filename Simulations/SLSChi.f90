Module constants  
  ...
  real, parameter :: threshhold_a1=-1.0,threshhold_a2=0.2,threshhold_a3=2.0,DF=4
  real, parameter :: CDF_a1=threshhold_a1*((DF*2)**0.5)+DF,CDF_a2=threshhold_a2*((DF*2)**0.5)+DF,CDF_a3=threshhold_a3*((DF*2)**0.5)+DF
...
program main
    ...
    open(10,file="D:/Location-Scale-Ordinal/Fortran_locationscale/TEXT/SLSChi_200223-L1N100.txt")
...
subroutine limitB(Limit,arl,std)
	...
	do i=1,simu
	  ...
	  do j = 1, ewma, 1	
	  call RNGET (iseed)  
      call RNCHI (sample_size, DF, sample)
	  !sample=sample+0.2
	  !print*,sample
	  sample=(sample-4.0)/(8.0**0.5)
...
subroutine ChiICProb(Prob)
	...
	interval(1)=CHIDF(CDF_a1,DF)
	interval(2)=CHIDF(CDF_a2,DF)
	interval(3)=CHIDF(CDF_a3,DF)
